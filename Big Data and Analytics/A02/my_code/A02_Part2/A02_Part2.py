...
def my_main(spark, my_dataset_dir, top_n_bikes):
    ...
    #count bike trips
    bikeTrips = inputDF.groupBy("bike_id").count()
    #bikeTrips.show()

    #aggregate trip duration
    tripDuration = inputDF.groupBy("bike_id").agg({'trip_duration':'sum'})
    #tripDuration.agg({'trip_duration':'sum'})#.show()


    bikeGrouping = tripDuration.join(bikeTrips, on=["bike_id"], how="inner")


    tempSolutionDF = bikeGrouping.withColumnRenamed("count","total_number_of_trips").withColumnRenamed("sum(trip_duration)", "totalTime").withColumnRenamed("total_number_of_trips", "numTrips")

    solutionDF = tempSolutionDF.orderBy('totalTime', ascending=False).limit(top_n_bikes)
    #solutionDF.show()
