...
# FUNCTION my_main
# ------------------------------------------
def my_main(spark, my_dataset_dir, year):
    ...
    validTrips = inputDF.filter(inputDF.birth_year >= 1990)
    validTrips = validTrips.filter(validTrips.user_type == "Subscriber")
    validTrips = validTrips.withColumn('start_hour', pyspark.sql.functions.hour(validTrips.start_time))

    trimmedValidTrips = validTrips.select("start_time", "birth_year", "start_hour")
    tempSolution = trimmedValidTrips.groupby("birth_year", "start_hour").count()
...
if __name__ == '__main__':
    ...
    my_local_path = "../../"
    my_databricks_path = "/FileStore/tables/6_Assignments/"

    my_dataset_dir = "my_dataset_1/"
    ...
    my_main(spark, my_dataset_dir, year)
