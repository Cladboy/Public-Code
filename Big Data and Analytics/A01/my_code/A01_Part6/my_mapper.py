...
def my_map(my_input_stream, my_output_stream, my_mapper_input_parameters):
    ...
    for line in my_input_stream:
        ...
        if trip_bike_id == my_mapper_input_parameters[0]:
    ...
    trip_dict_items = trip_dict.items()
    #trip_dict_items = trip_dict_items[1:]

    to_write = "universal\t("
    to_add = ""
    for trip in trip_dict_items:
        temp_formatting = str(trip[1]["start_time"]) + " @ " + str(trip[1]["finish_time"]) + " @ " + str(trip[1]["start_station"]) + " @ " + str(trip[1]["finish_station"]) + " @ "
        to_add += temp_formatting
    if len(to_add)>0:
        to_add = to_add[:-3]
        to_write += to_add + ")\n"
        my_output_stream.write(to_write)
