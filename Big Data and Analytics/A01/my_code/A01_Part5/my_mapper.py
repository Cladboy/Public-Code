...
def my_map(my_input_stream, my_output_stream, my_mapper_input_parameters):
    ...
    for line in my_input_stream:
        ...
        trip_duration = parameters[2]

        if (bike_id not in bike_dict):
            bike_dict[bike_id] = {"total_duration": trip_duration, "total_trips": 1}
        else:
            bike_dict[bike_id]["total_duration"] += trip_duration
            bike_dict[bike_id]["total_trips"] += 1

    bike_dict_sorted = bike_dict.items()
    bike_dict_sorted = sorted(bike_dict_sorted)


    for bike in bike_dict_sorted:
            output = "universal\t(" + str(bike[0]) + ", " + str(bike[1]["total_duration"]) + ", " + str(bike[1]["total_trips"]) + ")\n"
