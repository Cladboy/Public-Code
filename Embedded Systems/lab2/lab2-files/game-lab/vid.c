...
// copy old contents of fb from the buffer
  for (y = 0, j = oldstartCol; j < oldstartCol + w; y++, j++)
  {
    for (x = 0, i = oldstartRow; i < oldstartRow + h; x++, i++)
    {
      fb[i * 640 + j] = buff[x][y];
    }
  }
...
// Save pixels about to be overwritten by pacman image to buffer
        buff[yy][xx] = fb[i * 640 + j];
        fb[i*640 + j] = pixel;
